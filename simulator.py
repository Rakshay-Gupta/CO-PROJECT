...
# Loads a 32 bit word from memory
    ...
    #operations 
        if(f3==0x0 and f7==0x00):
            result=self.unsigned_32(v1+v2)          #add
        elif(f3==0x0 and f7==0x20):
            result=self.unsigned_32(v1-v2)          #sub 
        elif(f3==0x1 and f7==0x00):
            result=self.unsigned_32(v1<<(v2 & 0x1F))           #sll
        elif(f3==0x2 and f7==0x00):
            if(self.signed_32(v1)<self.signed_32(v2)):           #slt
                result=1         
            else:
                result=0
            ...
            result=self.unsigned_32(v1 ^ v2)       #xor
        elif(f3==0x5 and f7==0x00):
            result=v1>>(v2 & 0x1F)        #srl
        elif(f3==0x5 and f7==0x20):
            result=self.unsigned_32(self.signed_32(v1)>>(v2 & 0x1F))         #sra
        elif(f3==0x6 and f7==0x00):
            result=self.unsigned_32(v1 | v2)         #or
        elif(f3==0x7 and f7==0x00):
            result=self.unsigned_32(v1 & v2)           #and
        else:
    ...
    def execute_i_type(self,decoded):
        ...
        if(opcode==0x03):
            ...
            if(f3==0x0):                                 # ADDI
                ...
                if(self.signed_32(v1)<imm):
                    result=1
                else:
                    result=0
            elif(f3==0x3):                                 # SLTIU
                if(v1<self.unsigned_32(imm)):
                    result=1
                else:
                    result=0
            elif(f3==0x4):                                 # XORI
...
#this is function for S-type
    def execute_s_type(self,decoded):
        ...
        self.pc+=4
#this is function for B-type
    def execute_b_type(self,decoded):
        rs1,rs2,f3=decoded['rs1'],decoded['rs2'],decoded['funct3']
        instr=decoded['instr']
              # breaking down immediate
        imm_11=(instr>>7) & 0x1
        imm_4_1=(instr>>8) & 0xF
        imm_10_5=(instr>>25) & 0x3F
        imm_12=(instr>>31) & 0x1
        imm=self.signExtending(
            (imm_12<<12) | (imm_11<<11) | (imm_10_5<<5) | (imm_4_1<<1),13      # joining all parts to convert it into 13 bits
        )
        v1=self.registers[rs1]
        v2=self.registers[rs2]
                   # checking branch conditions wrt funct3(f3)
        if(f3==0x0):
            taken=(v1==v2)
        elif(f3==0x1):
            taken=(v1!=v2)
        elif(f3==0x4):
            taken=self.signed_32(v1)< self.signed_32(v2)
        elif(f3==0x5):
            taken=self.signed_32(v1)>=self.signed_32(v2)
        elif(f3==0x6):
            taken=v1<v2
            ...
            raise SystemExit(f"Error: unknown B-type f3={f3} at PC=0x{self.pc:08X}")       # unknown instruction error in this B-type
        if(taken): 
            self.pc=self.unsigned_32(self.pc+imm)      # means branch is taken
        else:
            self.pc+=4          # means condition not satisfied therefor, branch is not taken
#this is function for U-type
    def execute_u_type(self,decoded):
        rd=decoded['rd']
        instr=decoded['instr']
        imm=instr & 0xFFFFF000      # to get upper 20 bits
        if(decoded['opcode']==0x37):
            ...
            raise SystemExit(f"Error: unknown U-type opcode at PC=0x{self.pc:08X}")       # unknown instruction error in U-type
        self.pc+=4
#this is function for J-type
    def execute_j_type(self,decoded):
        rd=decoded['rd']
        instr=decoded['instr']
             # to get immediate at those bit places
        ...
        if(rd!=0):
            self.registers[rd]=self.unsigned_32(self.pc+4)       # to save return address in rd
        self.pc=self.unsigned_32(self.pc+imm)        # to jump to desired address
...
#now starting the virtual halt function 
    ...
    def execute(self,output_file):
        ...
        for _ in range(10_000_000):
            ...
            opcode=decoded['opcode']
            #checking konsa function hai opcode ka
            if(opcode==0x33):
                ...
                self.execute_u_type(decoded)
            elif(opcode==0x6F):
                self.execute_j_type(decoded)
            else:
